...
def calc_ENS(mpc_obj, sw_recloser, sw_sectionalizer, sw_automatic_sectioner, sw_manual_sectioner, sw_cutout,
    ...
    # initialization
    sw_recloser = np.array(sw_recloser)
    sw_sectionalizer = np.array(sw_sectionalizer)
    sw_automatic_sectioner = np.array(sw_automatic_sectioner)
    sw_manual_sectioner = np.array(sw_manual_sectioner)
    sw_cutout = np.array(sw_cutout)
    ...
    for h in range(1, mpc_obj.branch.shape[0] + 1):
        ...
        temp_a = int(mpc_obj.branch.iloc[faulted_branch[0] - 1, 0]) - 1
        temp_b = int(mpc_obj.branch.iloc[faulted_branch[0] - 1, 1]) - 1
        ...
        repair_time = get_dist(current_xy, fault_xy) / speed + mpc_obj.branch_reliability.iloc[faulted_branch[0] - 1, 1]
        current_xy_repair_team = fault_xy  # Current location of the repair team

        restoration_time = mpc_obj.branch_fault_allocation_time.iloc[0,
                                                                     faulted_branch[
                                                                         0] - 1] + time_to_reach_to_faulty_point + isolation_time
        ENS0 = lost_power_before_maneuver * (
                mpc_obj.branch_fault_allocation_time.iloc[0,
                                                          faulted_branch[
                                                              0] - 1] + time_to_reach_to_faulty_point + isolation_time)
        ...
        if len(final_livebus_ordered) == 0:
            maneuvering_time = repair_time * 10
        else:
            current_xy_maneuvering_team = current_xy  # current loc of manuovering team

            for i in range(len(final_livebus_ordered)):
                if (final_livebus_ordered.shape[0] == 1):
                    final_livebus_ordered = np.reshape(final_livebus_ordered, (1, -1))
                    final_livebus_ordered = np.r_[final_livebus_ordered, np.zeros((1, final_livebus_ordered.shape[1]))]
                ...
                if livebus_auto[np.where(livebus_loc == final_livebus_ordered[i])] == 1:
                    ...
                    final_livebus_ordered[1, i] = get_dist(current_xy_maneuvering_team,
                                                           mpc_obj.bus_xy.iloc[final_livebus_ordered[0, i] - 1,
                                                           :]) / speed
                    current_xy_maneuvering_team = mpc_obj.bus_xy.iloc[final_livebus_ordered[1, i] - 1, :]

            maneuvering_time = sum(final_livebus_ordered[1, :])
        ...
        if repair_time > maneuvering_time:
            ...
            for i in range(len(final_livebus_ordered[0, :]) - 1):
            ...
            for i in range(len(final_livebus_ordered[0, :]) - 1, -1, -1):
                if final_livebus_ordered[1, i] != 0:
                    final_livebus_ordered[1, i] = get_dist(current_xy_maneuvering_team, mpc_obj.bus_xy
                                                           .iloc[final_livebus_ordered[0, i] - 1, :]) / speed
                    current_xy_maneuvering_team = mpc_obj.bus_xy.iloc[final_livebus_ordered[1, i] - 1, :]

            for i in range(len(final_livebus_ordered[1, :]) - 1, -1 - 1):
            ...
            tot_ENS += ENS0 * mpc_obj.branch_reliability.iloc[faulted_branch[0] - 1, 0]
        else:
            ...
            tot_ENS += lost_power_before_maneuver * (
                    repair_time + isolation_time + UnIsolation_time) * mpc_obj.branch_reliability.iloc[
