...
def optimize_ens(mpc_obj, livebus_loc, livebus_auto, sw_recloser_exist, sw_sectionalizer_exist,
    ...
    sw_cutout_exist = np.array(current_xy)
...
def opt_fun(branch, mpc_obj, switches_arr, sw_recloser_exist, sw_sectionalizer_exist,
    ...
    sw_sectionalizer_current = np.append(sw_sectionalizer_exist, branch[
                                                                 switches_arr[0] + switches_arr[1]:switches_arr[0] +
                                                                                                   switches_arr[1] +
                                                                                                   switches_arr[2]])
    sw_sectioner_manual_current = np.append(sw_sectioner_manual_exist, branch[
                                                                       switches_arr[0] + switches_arr[1] + switches_arr[
                                                                           2]:switches_arr[0] + switches_arr[1] +
                                                                              switches_arr[2] + switches_arr[3]])
    sw_sectioner_automatic_current = np.append(sw_sectioner_automatic_exist, branch[switches_arr[0] + switches_arr[1] +
                                                                                    switches_arr[2] + switches_arr[3]:
                                                                                    switches_arr[0] + switches_arr[1] +
                                                                                    switches_arr[2] + switches_arr[3] +
                                                                                    switches_arr[4]])
...
def optimize(mpc_obj, avail_branches, switches_arr, sw_recloser_exist, sw_sectionalizer_exist,
    ...
    a = np.apply_along_axis(opt_fun, axis=1, arr=br_combs, mpc_obj=mpc_obj, switches_arr=switches_arr,
                            sw_recloser_exist=sw_recloser_exist,
